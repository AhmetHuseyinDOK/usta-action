...
async function fuzzyMatchSpec(
  ...
  try {
    ...
    const caseInsensitiveMatch = directories.find(
      (dir) => dir.toLowerCase() === lowerSearchTerm,
    );
    ...
    const fuzzyMatches = directories.filter(
      (dir) =>
    ...
    if (fuzzyMatches.length === 1 && fuzzyMatches[0]) {
      ...
      throw new Error(
        `Multiple specs found matching '${searchTerm}': ${fuzzyMatches.join(", ")}`,
      );
    }
...
export async function getSpecPath(
  ...
  throw new Error(
    `Spec not found: ${specName}. Searched in standard location and current directory.`,
  );
}
  ...
  for (const line of lines) {
    ...
    if (currentTask && line.match(/^  /)) {
      ...
      if (trimmedLine.startsWith("_Requirements:")) {
        const reqText = trimmedLine
          .replace(/^_Requirements:\s*/, "")
          .replace(/_$/, "");
